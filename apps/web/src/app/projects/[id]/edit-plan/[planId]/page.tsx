...
export default function RoughCutPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(\http://127.0.0.1:8000/api/v1/projects/\/edit-plan/plans/\\);
        if (res.ok) {
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Rough Cut</h1>
            <p className="text-muted-foreground mt-2">
              Review the clips selected for Plan {plan.id.substring(0, 8)}
            </p>
          </div>
          <Link
            href={\/projects/\/edit-plan\}
